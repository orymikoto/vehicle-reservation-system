...
export const Badge: React.FC<BadgeProps> = ({ status, className = '' }) => {
  ...
  if (['APPROVED', 'AVAILABLE', 'COMPANY'].includes(normalized)) {
    styles = 'bg-emerald-50 text-emerald-800 border-emerald-200';
  } else if (['PENDING', 'RESERVED', 'ON_DUTY', 'RENTAL'].includes(normalized)) {
    styles = 'bg-amber-50 text-amber-800 border-amber-200';
  } else if (['REJECTED', 'INACTIVE', 'CANCELLED'].includes(normalized)) {
    styles = 'bg-red-50 text-red-800 border-red-200';
  } else if (['MAINTENANCE', 'COMPLETED'].includes(normalized)) {
